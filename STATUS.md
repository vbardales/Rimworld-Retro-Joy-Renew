...
dependencies:   none
showcase:       preview checked; ModIcon pending user generation
tested_on:
workshop:
remaining:
  - unverified: user-generated ModIcon.png is required for horsMonoRepo → ModIcon générée
...
**Current stage: `horsMonoRepo`.** RetroJoyRenew is now an autonomous `main` repository connected to
...
Plucker. `About/ModIcon.png` is intentionally absent pending user generation; no generated image
belongs in this checkout until then.
...
link. Options, localization, no-dependency declaration, tests and Pickle scenarios remain valid
independent evidence, but the absent ModIcon blocks the ordered workflow before `preTest`.
...
To reach `ModIcon générée`, add the ModIcon generated by the user at `Mod/About/ModIcon.png`, then
inspect it at 128 px and 32 px before advancing. The later Pickle execution remains pending and must
use the shared WSL launcher and machine lock; Windows RimWorld must not be launched.
